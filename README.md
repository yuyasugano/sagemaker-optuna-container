# scikit-learn with optuna container sample

This example shows how to package an algorithm for use with SageMaker. We have chosen a simple [scikit-learn][skl] implementation with [optuna][optn] to illustrate the procedure.
...
* __gradient-boost__: The directory that contains the application to run in the container. See the next session for details about each of the files under optuna directory.
...
* __payload.csv__: Sample 10 records data for used by predict.sh for testing the server.
...
[optn]: https://optuna.readthedocs.io/en/latest/
